...
The other connection parameters are **host**(default: localhost), **port**(default: 9042) and **keyspace**(default: empty). When specifying the keyspace this will be the default keyspace used for operations if is is not otherwise specified.
