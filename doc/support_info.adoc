...
. **Rename column** operation accepts the table name as a parameter and a Map<String, String> as payload. The map will be containing the name of the column to be renamed as  akey and as a value the new name for the column.
+
Example:
[source]
%dw 1.0
...
[source]
{
	"my_old_column_name_1":"my_new_column_name_1",
